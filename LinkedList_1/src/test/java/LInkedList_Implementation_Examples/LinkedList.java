...
public class LinkedList
{
    ...
    {
        if(index == 0) // что если мы захотим удалить нулевой индекс?
        { // в этом случае нам надо будет изменить местоположение нашего head
            head = head.next;
        }
        else
        {
            ...
            Node n1 = null;
            for(int i=0; i<index-1; i++)
            {   // из начала списка с head мы продвигаемся к нужной позиции
                n = n.next;
            }
            n1 = n.next; // затем n.next должно быть замещено n1.next
            n.next = n1.next;
            // System.out.println("n1 " + n1.data);
            n1 = null; // если хотим удалить как бы значение еще до сборщика мусора
        }
    }
